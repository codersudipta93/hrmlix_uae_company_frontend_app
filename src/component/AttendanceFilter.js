...
const AttendanceFilter = (props) => {
    ...
    useEffect(() => {
        if (props?.filterParamData) {
            ...
            setHod(HelperFunctions.updateSelectedArrObjects(hodData, pdata?.hodData, '_id'))
        ...
        postApi("company/get-employee-master", {}, token)
            .then((resp) => {
                ...
                if (resp?.status == 'success') {
                    ...
                    // alert(props?.filterParamData)
                    if (props?.filterParamData) {
                        let pdata = props?.filterParamData;
                        console.log("Data from attendance page ====> ", pdata);

                        // setSearchVal(pdata?.searchkey);
                        // setClients(HelperFunctions.updateSelectedArrObjects(mData?.clients, pdata?.client_id, '_id'))
                        // setSelectedClient(pdata?.client_id);

                        // setDepartment(HelperFunctions.updateSelectedArrObjects(mData?.department, pdata?.department_id, '_id'))
                        // setSelectedDepartment(pdata?.department_id);
                        ...
                        // setDesignation(HelperFunctions.updateSelectedArrObjects(mData?.designation, pdata?.designation_id, '_id'))
                        // setSelectedDesignation(pdata?.designation_id);

                        // setBranch(HelperFunctions.updateSelectedArrObjects(mData?.branch?.company_branch, pdata?.branch_id, '_id'))
                        // setSelectedBranch(pdata?.branch_id);

                        // setHod(HelperFunctions.updateSelectedArrObjects(mData?.hod, pdata?.hod_id, '_id'))
                        // setSelectedHod(pdata?.hod_id);
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        <View style={{ paddingHorizontal: 14, marginTop: 12 }}>
                            ...
                            <View style={{ padding: 12, backgroundColor: colors.white, borderRadius: 8, marginBottom: 20 }}>
                                {props?.modalFor == "attendance_page" ?
                                    <FloatingLabelInput
                                        label="Search"
                                        placeholder="Search"
                                ...
                                {props?.modalFor == "attendanceView_page" ?
