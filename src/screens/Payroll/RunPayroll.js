...
const RunPayroll = props => {
  ...
  useEffect(() => {
    ...
    if (filterdata != null) {
      ...
      setIsLoading(true);
      postApi("company/generate-earning-sheet-employee-list", filterdata, token)
        .then((resp) => {
          console.log(resp);
  ...
  const openFilter = () => {
    ...
    props.navigation.navigate('FilterPayroll', { paramData: pData })
    ...
    <View style={[styles.listCard, { paddingVertical: 22, marginBottom: 0, borderRadius: 0, borderTopRightRadius: index == 0 ? 8 : 0, borderTopLeftRadius: index == 0 ? 8 : 0, borderBottomLeftRadius: sampleData.length - 1 == index ? 8 : 0, borderBottomRightRadius: sampleData.length - 1 == index ? 8 : 0 }]}>
      <View style={{ width: '60%', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
        ...
        <View style={{ paddingLeft: 12 }}>
          <Text style={{ fontFamily: FontFamily.medium, color: '#4E525E', fontSize: sizes.h6, textAlign: 'left' }}>ss{item?.emp_first_name} {item?.emp_last_name}</Text>
      ...
      >
        <View>
          <View style={{ marginBottom: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={[styles.modalheadingText,{}]}>Details</Text>
            <IonIcon
              name="close"
              size={20}
              color="#4E525E"
          ...
          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View><Text style={styles.leftText}>Attendance Record</Text></View>
            <View><Text style={[styles.rightText, { textTransform: 'capitalize' }]}>{selectedList?.attendance_summaries ? selectedList?.attendance_summaries[0]?.paydays :'N/A'}</Text></View>
          </View>

          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View><Text style={styles.leftText}>Department</Text></View>
            <View><Text style={[styles.rightText, { textTransform: 'capitalize' }]}>{selectedList?.department?.department_name ? selectedList?.department?.department_name : 'N/A'}</Text></View>
          </View>

          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View><Text style={styles.leftText}>Branch</Text></View>
            <View><Text style={[styles.rightText, { textTransform: 'capitalize' }]}>{selectedList?.branch?.branch_name ? selectedList?.branch?.branch_name  : 'N/A'}</Text></View>
          </View>

          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View><Text style={styles.leftText}>Client</Text></View>
            <View><Text style={[styles.rightText, { textTransform: 'uppercase' }]}>{selectedList?.client_code ? HelperFunctions.getNameById(masterData?.clients, selectedList?.client_code, 'client_name') != "Not found" ? HelperFunctions.getNameById(masterData?.clients, selectedList?.client_code, 'client_name') :"N/A": 'N/A'}</Text></View>
          </View>
         
          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View><Text style={styles.leftText}>Status</Text></View>
            <View><Text style={[styles.rightText, { textTransform: 'uppercase' }]}>{selectedList?.client?.client_code ? selectedList?.client?.client_code  : 'N/A'}</Text></View>
          </View>
