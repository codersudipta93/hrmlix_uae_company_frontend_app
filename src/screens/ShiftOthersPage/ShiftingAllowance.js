...
const ShiftingAllowance = props => {
  ...
  const openFilter = () => {
    ...
    let pData = {
      ...
      "emp_name": props?.route?.params?.paramData?.emp_name ? props?.route?.params?.paramData?.emp_name : "",
      "page_component_name": "ShiftingAllowance"
    }
  ...
  const shiftRateRender = ({ index, item }) => (
    <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
      <View><Text style={styles.leftText}>{HelperFunctions.getNameById(activeShift, item?.shift_id, 'shift_name')}</Text></View>
...
export default ShiftingAllowance;