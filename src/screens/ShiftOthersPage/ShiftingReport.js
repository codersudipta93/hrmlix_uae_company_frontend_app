...
const ShiftingReport = props => {
  ...
  useEffect(() => {
    ...
    if (filterdata != null) {
      ...
      setIsLoading(true);
      postApi("company/shift-duty-report", filterdata, token)
        .then((resp) => {
          console.log(resp);
  ...
  const openFilter = () => {
    ...
    let pData = {
      ...
      "page_component_name" : "ShiftingReport"
    }
    ...
    <SafeAreaView style={styles.main}>
      <View style={styles.main}>
       ...
       <CustomHeader hideUserIcon={true}
          buttonText={t('Shift Duty Report')}
      ...
      >
        <View>
          <View style={{ marginBottom: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={[styles.modalheadingText,{}]}>{HelperFunctions.getDateDDMMYY(selectedShift?.shift_data?.from)} - {HelperFunctions.getDateDDMMYY(selectedShift?.shift_data?.to)}</Text>
            <IonIcon
              name="close"
              size={20}
              color="#4E525E"
          ...
          <View style={{ marginBottom: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <View><Text style={[styles.rightText, { textTransform: 'uppercase' }]}>{selectedShift?.shift_data?.shift_name ? selectedShift?.shift_data?.shift_name : 'N/A'}</Text></View>
          </View>
