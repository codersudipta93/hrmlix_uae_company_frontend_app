...
const TrainingDetails = props => {
 ...
 // useFocusEffect(
  ...
  const showImage = (item) => {
    ...
    if (item?.training_file_image) {
      setIsLoading(true);
      let param = { image_path: item?.training_file_image }
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <View>
              <Text style={{ textTransform: 'capitalize', fontFamily: FontFamily.medium, color: index == selectedIndex ? '#fff' : '#4E525E', fontSize: sizes.md + 1, textAlign: 'left' }}>
                {item?.training_type ? item?.training_type : "N/A"}
              </Text>
              ...
              props?.route?.params?.paramData?.emp_det?.training != "" ?
                <>
                  <FlatList
                    ...
                    data={props?.route?.params?.paramData?.emp_det?.training}
