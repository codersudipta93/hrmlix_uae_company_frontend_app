...
const ContractDetails = props => {
  ...
  const showImage = (item) => {
    ...
    if (item?.contract_file_image) {
      setIsLoading(true);
      let param = { image_path: item?.contract_file_image }
    ...
    <>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <View>
              <Text style={{ textTransform: 'capitalize', fontFamily: FontFamily.medium, color: index == selectedIndex ? '#fff' : '#4E525E', fontSize: sizes.md + 1, textAlign: 'left' }}>
                 {item?.start_date ? HelperFunctions.getDateDDMMYY(item?.start_date) : "N/A"} - {item?.end_date ? item?.end_date : "N/A"} 
              </Text>
      ...
      {index == selectedIndex ?
        <View style={{ paddingLeft: 6 }}>
          ...
          <View style={{ flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', padding:12 }}>
            <View>
              <Text style={[styles.optionname, { marginTop: 8, fontFamily: FontFamily.medium }]}>Comments</Text>
            </View>
            <View style={{ paddingRight: 12 }}>
              <Text style={[styles.optionVal, { textTransform: 'capitalize', fontSize: sizes.md, marginTop: 6, fontFamily: FontFamily.regular }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={{ paddingHorizontal: 14, flexDirection: 'column', justifyContent: 'space-between', marginTop: 12 }}>
            ...
            {isLoading ? (
              ...
              // Show actual data
              props?.route?.params?.paramData?.emp_det?.contract != "" ?
                <>
                  <FlatList
                    showsVerticalScrollIndicator={false}
                    data={props?.route?.params?.paramData?.emp_det?.contract}
