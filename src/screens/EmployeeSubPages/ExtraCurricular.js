...
const ExtraCurricular = props => {
  ...
  const showImage = (item) => {
    ...
    if (item?.extracurriculum_file_image) {
      setIsLoading(true);
      let param = { image_path: item?.extracurriculum_file_image }
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
            <View>
              <Text style={{ textTransform: 'capitalize', fontFamily: FontFamily.medium, color: index == selectedIndex ? '#fff' : '#4E525E', fontSize: sizes.md + 1, textAlign: 'left' }}>
                {item?.extra_curricular_type ? item?.extra_curricular_type : "N/A"}
              </Text>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={{ paddingHorizontal: 14, flexDirection: 'column', justifyContent: 'space-between', marginTop: 12 }}>
            ...
            {isLoading ? (
              ...
              // Show actual data
              props?.route?.params?.paramData?.emp_det?.extra_curricular != "" ?
                <>
                  <FlatList
                    showsVerticalScrollIndicator={false}
                    data={props?.route?.params?.paramData?.emp_det?.extra_curricular}
