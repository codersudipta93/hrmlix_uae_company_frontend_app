...
const CalculatedRevision = props => {
  ...
  const _updatefilterData = () => {
    ...
    if (props?.route?.params) {
      ...
      let apiParam = {
        ...
        "pageno": 1,
        "searchkey": "",
        //"month": 8, // update from rahul on 23-09-24
        //"year": 2024, // update from rahul on 23-09-24
  ...
  useEffect(() => {
    ...
    if (filterdata != null) {
      setIsLoading(true);
      postApi("company/get-filtered-revision-emp-list", filterdata, token)
        .then((resp) => {
  ...
  const runCalculateRivision = () => {
    setBtnLoaderStatus(true);
    let param = {
      row_checked_all: false,
      checked_row_ids: JSON.stringify((empData?.docs || [])
    ...
    console.log(param)
    postApi("company/create-revision-schedule", param, token)
      .then((resp) => {
        if (resp?.status == 'success') {
  ...
  const _openFilter = () => {
    ...
    setIndex(null)
    props.navigation.navigate('CalculatedRevisionFilter', { paramData: pData })
  }
        ...
        <View>
          <View style={styles.listContainer}>
            ...
            <View style={{ paddingRight: 12 }}>
              <Text style={[styles.optionVal, { textTransform: 'uppercase' }]}>
                (AED) {item?.revision?.gross_salary}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.listContainer}>
            ...
            <View style={{ paddingRight: 12 }}>
              <Text style={styles.optionVal}>
                {HelperFunctions.getNameById(masterData?.salarytemp, item?.revision?.salary_temp_id, 'template_name')}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.listContainer}>
            ...
            <View style={{ paddingRight: 12 }}>
              <Text style={styles.optionVal}>
                {HelperFunctions.getNameById(masterData?.packages, item?.revision?.policy_pack_id)}
              </Text>
            </View>
          </View>
          <View style={styles.listContainer}>
          ...
          <View style={styles.listContainer}>
            <View>
              <Text style={styles.optionname}>Revision Run Status</Text>
            </View>
            <View style={{ paddingRight: 12 }}>
              <Text style={[styles.optionVal, { textTransform: 'capitalize' }]}>{item?.revision_run_status}</Text>
            </View>
          </View>
        </View>
        : null}
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={{ paddingHorizontal: 14, flexDirection: 'column', justifyContent: 'space-between', marginTop: 12 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, }}>
              ...
              <Text style={{ fontFamily: FontFamily.semibold, color: '#4E525E', fontSize: sizes.h6, lineHeight: 35, }}>Calculated Revision</Text>
                ...
                {(empData?.docs || [])
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      runCalculateRivision()
                      : null
                    }}
                    style={{ width: '20%', borderRadius: 4, opacity: btnLoaderStatus == true ? 0.6 : 1, backgroundColor: colors.primary, paddingVertical: 5 }}>
                    <Text style={{ textAlign: 'center', letterSpacing: 1.2, fontFamily: FontFamily.medium, color: '#fff', fontSize: sizes.h6, textTransform: 'capitalize' }}>Run</Text>
                  </TouchableOpacity>
...
export default CalculatedRevision;